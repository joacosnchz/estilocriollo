<?php

global $_MODULE;
$_MODULE = array();

$_MODULE['<{statsequipment}prestashop>statsequipment_247270d410e2b9de01814b82111becda'] = 'Exploradores y sistemas operativos';
$_MODULE['<{statsequipment}prestashop>statsequipment_2876718a648dea03aaafd4b5a63b1efe'] = 'Añade una pestaña con gráficos acerca del uso del navegador web y el sistema operativo al panel de control de estadísticas.';
$_MODULE['<{statsequipment}prestashop>statsequipment_6602bbeb2956c035fb4cb5e844a4861b'] = 'Guia';
$_MODULE['<{statsequipment}prestashop>statsequipment_854c8e126f839cc861cde822b641230e'] = 'Asegúrate de que tu web está disponible para tanta gente como sea posible';
$_MODULE['<{statsequipment}prestashop>statsequipment_00607ff6337f1ee20badf91b3e5042b3'] = 'Cuando se administran páginas web, es importante no perder de vista el software utilizado por los visitantes con el fin de asegurarse de que la página se muestra de manera idéntica para todos. PrestaShop fue creado para ser compatible con la mayoría de los navegadores recientes y de los sistemas operativos de ordenadores (SO). Sin embargo, puesto que puedes añadir características avanzadas a tu web o incluso modificar el código de base de PrestaShop, estas modificaciones pueden no ser accesibles para todos. Por esa razón es una buena idea guardar en pestañas una relación del porcentaje de usuarios para cada tipo de software antes de hacer algún cambio al que sólo podrá acceder un número limitado de usuarios.';
$_MODULE['<{statsequipment}prestashop>statsequipment_11db1362a88c5e3e74c8f699c14d6798'] = 'Indica el porcentaje de cada navegador utilizado por los clientes.';
$_MODULE['<{statsequipment}prestashop>statsequipment_998e4c5c80f27dec552e99dfed34889a'] = 'Exportar CSV';
$_MODULE['<{statsequipment}prestashop>statsequipment_90c58bfe4872fc9ca7bf6a181c3e5edd'] = 'Indica el porcentaje de cada sistema operativo utilizado por los clientes.';
$_MODULE['<{statsequipment}prestashop>statsequipment_bb38096ab39160dc20d44f3ea6b44507'] = 'Plug-ins';
$_MODULE['<{statsequipment}prestashop>statsequipment_9ffafc9e090c8e1c06f928ef2817efd6'] = 'Navegador utilizado';
$_MODULE['<{statsequipment}prestashop>statsequipment_0241b7aaaa5f76afd585bb6cdae314d1'] = 'Sistema operativo utilizado';


return $_MODULE;
