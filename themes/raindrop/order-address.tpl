...
<div class="frame_wrap frame_wrap_header clearfix">
    <div class="fwh-title">{l s='Addresses'}</div>
</div>
<div class="frame_wrap frame_wrap_content clearfix">
    ...
    {else}
        ...
        <div class="frame_wrap frame_wrap_header clearfix">
            <div class="fwh-title"><span>1</span> {l s='Addresses'}</div>
        </div>
        <div class="frame_wrap frame_wrap_content clearfix">
            ...
            <div class="addresses clearfix">
                <div class="row">
                    <div class="col-xs-12 col-sm-12">
                        ...
                        <div class="addressesAreEquals"{if $cart->isVirtualCart()} style="display:none;"{/if}>
                    ...
                    <div class="col-xs-12 col-sm-12">
                        <div id="address_invoice_form" class="select form-group selector1"{if $cart->id_address_invoice == $cart->id_address_delivery} style="display: none;"{/if}>
                            ...
                            {else}
                                <a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="button btn no-border-radius">
                                    <span>
                ...
                <p class="address_add submit">
                    <a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Add'}" class="button btn no-border-radius">
            ...
            {if !$opc}
                <p class="cart_navigation clearfix">
                    ...
                    <a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")|escape:'html':'UTF-8'}" title="{l s='Previous'}" class="button-exclusive btn button no-border-radius">
                        {l s='Continue Shopping'}
                    </a>
                    <button type="submit" name="processAddress" class="btn btn-primary exclusive no-border-radius standard-checkout">
                        <span>{l s='Proceed to checkout'}</span>
                    </button>
                </p>
                </form>

            </div> <!-- end frame_wrap_content 5 steps -->

        {else}
        </div> <!--  end opc_account -->

</div> <!-- end frame_wrap_content opc -->
{/if}
{strip}
    {if !$opc}
...
{capture}<a class="button btn-sm btn no-border-radius" href="{$smarty.capture.addressUrlAdd}" title="{l s='Update' js=1}"><span>{l s='Update' js=1}</span></a>{/capture}
